...
const RequestSchema = z.object({
  ...
  voice: z
    .string()
    .refine((v) => VOICES.has(v), { message: "Unsupported voice" }),
  ...
  while (i < sents.length) {
    ...
    if (bytes > MAX_BYTES) {
      ...
      while (lo <= hi) {
        ...
        if (Buffer.byteLength(tmp) <= MAX_BYTES) {
          best = mid;
          lo = mid + 1;
        } else {
          hi = mid - 1;
  ...
  while (start < totalWords) {
    ...
    if (!started) {
      ...
      const nextWord = tokens.find(
        (t) => t.t === "word" && (t.wi ?? Infinity) >= start,
      );
...
export async function POST(req: NextRequest) {
  ...
  if (
    ...
    return NextResponse.json(
      {
        error:
          "Server not configured: set GOOGLE_PROJECT_ID, GOOGLE_CLIENT_EMAIL, GOOGLE_PRIVATE_KEY",
      },
      { status: 500 },
    );
  }
  ...
  try {
    ...
    const effectivePreviewSec = entitled
      ? 0
      : typeof previewSec === "number"
        ? previewSec
        : preview
          ? PREVIEW_SECONDS_DEFAULT
          : 0;
    if (DEBUG_TTS)
      console.log(
        "[TTS] request: voice=%s speed=%s len=%d format=%s",
        voice,
        speed,
        text.length,
        format,
    ...
    const endExclusive = Math.min(
      totalChunks,
      maxChunks ? start + maxChunks : totalChunks,
    );
    if (DEBUG_TTS)
      console.log(
        "[TTS] tokens=%d chunks=%d range=%d..%d",
        totalWords,
        totalChunks,
        start,
        endExclusive,
    ...
    if (format === "audio") {
      ...
      for (let i = start; i < endExclusive; i++) {
        ...
        if (DEBUG_TTS)
          console.log(
            "[TTS] audio seg %d dur=%dms bytes=%d",
            i,
            durationMs,
            buf.byteLength,
          );
        if (effectivePreviewSec > 0) {
          const soFar = segments.reduce(
            (a, s) => a + Math.max(0, s.durationMs || 0),
            0,
          );
    ...
    if (format === "marks") {
      ...
      for (let i = 0; i < chunks.length; i++) {
        ...
        const tps = ((resp as any).timepoints || []) as {
          markName: string;
          timeSeconds: number;
        }[];
        for (const tp of tps) {
          marks.push({
            type: "word",
            time: offsetMs + Math.round(Number(tp.timeSeconds) * 1000),
            value: tp.markName,
          });
        }
        if (DEBUG_TTS)
          console.log(
            "[TTS] marks seg %d count=%d dur=%dms",
            i,
            tps.length,
            durationMs,
          );
    ...
    for (let i = start; i < endExclusive; i++) {
      ...
      if (isChirpVoice(voice)) {
        ...
        if (includeMarks) {
          ...
          const refAudioContent = refResp.audioContent as
            | Buffer
            | Uint8Array
            | string;
          ...
          try {
            const metaR = await mm.parseBuffer(refBuf, {
              mimeType: "audio/mpeg",
            });
            refDurationMs = Math.round((metaR.format.duration || 0) * 1000);
          } catch {}
          const refTimepoints = ((refResp as any).timepoints || []) as {
            markName: string;
            timeSeconds: number;
          }[];
          scaledLocal = refTimepoints.map((tp) =>
            Math.round(Number(tp.timeSeconds) * 1000),
          );
        }
        ...
        const tgtAudioContent = tgtResp.audioContent as
          | Buffer
          | Uint8Array
          | string;
        ...
        for (let w = 0; w < count; w++) {
          const local =
            includeMarks && typeof scaledLocal[w] === "number"
              ? scaledLocal[w]
              : w * per;
          const tGlobal = Math.max(prev + 1, offsetMs + local);
          if (includeMarks)
            allTimepoints.push({ wordIndex: c.startWord + w, tMs: tGlobal });
          prev = tGlobal;
        }
        ...
        segments.push({
          idx: i,
          audioBase64: tgtBuf.toString("base64"),
          durationMs: chunkDur,
          startWordIndex: c.startWord,
          mime: targetMime,
        });
        if (DEBUG_TTS)
          console.log(
            "[TTS] chunk %d: refDur=%dms tgtDur=%dms chunkDur=%dms marks=%d",
            i,
            refDurationMs,
            tgtDurationMs,
            chunkDur,
        ...
        offsetMs += chunkDur;
        if (effectivePreviewSec > 0 && offsetMs >= effectivePreviewSec * 1000)
          break;
      } else {
        ...
        if (includeMarks) {
          const timepointsRaw = ((response as any).timepoints || []) as {
            markName: string;
            timeSeconds: number;
          }[];
          localTimes = timepointsRaw.map((tp) =>
            Math.round(Number(tp.timeSeconds) * 1000),
          );
        }
        ...
        for (let w = 0; w < count; w++) {
          const local =
            includeMarks && typeof localTimes[w] === "number"
              ? localTimes[w]
              : w * per;
          const tGlobal = Math.max(prev + 1, offsetMs + local);
          if (includeMarks)
        ...
        if (DEBUG_TTS)
          console.log(
            "[TTS] chunk %d: duration=%dms marks=%d",
            i,
            durationMs,
            count,
          );
