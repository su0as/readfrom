...
export async function GET(req: NextRequest) {
  try {
    ...
    const container = (
      searchParams.get("container") === "ogg" ? "ogg" : "mp3"
    ) as "mp3" | "ogg";
    ...
    if (!r.ok) {
      ...
      return NextResponse.json(
        { error: "TTS failed", details: snippet || undefined },
        { status: 500 },
      );
    }
    let j: any = null;
    try {
      ...
      const snippet = raw ? raw.slice(0, 500) : "";
      return NextResponse.json(
        { error: "TTS returned non-JSON", details: snippet || undefined },
        { status: 500 },
      );
    }

    const segs = (j?.segments || []) as {
      audioBase64: string;
      mime?: string;
    }[];
