...
export default function PricingSidebar({
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      {entitled && (
        <div className="badge" title="Your plan is active">
          Active plan
        </div>
      )}
      <div className="toggle" aria-label="Billing">
        {(["monthly", "yearly"] as Billing[]).map((b) => (
          ...
          >
            {b === "yearly" ? (
              <>
                Yearly <span className="opacity-80">(save {SAVE.pro}%)</span>
              </>
            ) : (
              "Monthly"
            )}
      ...
      <Link className="text-sm underline opacity-80" href="/pricing">
        See all plans
      </Link>
    </div>
  );
